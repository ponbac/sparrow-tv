...
The packaged Hyprland comparison found working in-app video after correcting audio codec wire names, but uneven delivery remains on interlaced samples. Choosing MSE as default is an accepted product tradeoff, not evidence of equivalent smoothness. See the [measurements](../research/linux-in-app-playback-hyprland.md). Improve frame pacing without changing the user's default to an external window. A subsequent [diagnosis](../research/linux-in-app-frame-pacing.md) isolated repeated latency-chasing seeks and buffer starvation; the initial threshold correction retained three seconds after catch-up instead of half a second. The [startup follow-up](../research/linux-in-app-startup-buffering.md) replaces append-triggered chasing with an owned timer: observe 1.5 seconds of approximately real-time delivery before catching up above ten seconds, retain five seconds for batched delivery, and allow at most one catch-up per 30 seconds. Playback starts immediately while the initial burst settles; a catch-up can still make one visible jump. The adapter cancels the timer before releasing or replacing its player.
